...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public List<ShoppingCartResponse> getAllShoppingCartByUserId(Long id) {
        return shoppingCartRepository.findAllByUserId(id)
                .stream().map(shoppingCartMapper::toDto).toList();
    }
    ...
    public void saveShoppingCart(ShoppingCartSaveRequest request) {
        ...
        shoppingCart.setCount(request.count());
        shoppingCart.setUser(userService.getUserById(request.userId()));
        shoppingCart.setProduct(productService.getProduct(request.productId()));
